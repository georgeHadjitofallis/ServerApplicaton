...
namespace ServerApplicaton
{
    class Database
    {
        ...
        {
            System.IO.Stream stream = File.Open(PATH_DATA + PATH_ACCOUNT + "/" + Network.Player[index].Username + FILE_EXTENSION, FileMode.OpenOrCreate);
            BinaryFormatter bf = new BinaryFormatter();

            bf.Serialize(stream, Network.Player[index]);
            stream.Close();

            MySQL.mysql.DB_RS.Open("SELECT * FROM accounts WHERE 0=1", MySQL.mysql.DB_CONN, CursorTypeEnum.adOpenStatic, LockTypeEnum.adLockOptimistic);

            var account = MySQL.mysql.DB_RS;
            {
                account.AddNew();
                account.Fields["Username"].Value = Network.Player[index].Username;
                account.Fields["Password"].Value = Network.Player[index].Password;
                account.Update();
            }

            MySQL.mysql.DB_RS.Close();
        }
